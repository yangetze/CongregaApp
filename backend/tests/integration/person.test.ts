...
describe("Person API Integration Tests", () => {
    ...
    it("should create a person and calculate age dynamically", async () => {
        ...
        const res = await request(app).post("/api/persons").send(payload);
        expect(res.status).toBe(201);
        expect(res.body).toHaveProperty("id");

        const getRes = await request(app).get(`/api/persons/document/DOC-123?organizationId=${orgId}`);
    ...
    it("should silently return existing id when duplicate documentId is created in the same org", async () => {
        ...
        const res1 = await request(app).post("/api/persons").send(payload);
        ...
        const res2 = await request(app).post("/api/persons").send(payloadDuplicate);
    ...
    it("should search and filter directory", async () => {
        ...
        const resList = await request(app).get(`/api/persons?organizationId=${orgId}&search=john`);
        expect(resList.status).toBe(200);
        expect(resList.body.length).toBe(1);
        expect(resList.body[0].firstName).toBe("John");

        const resListEmpty = await request(app).get(`/api/persons?organizationId=${orgId}&search=nonexistent`);
    ...
    it("should establish bidirectional relationships and reject self-relation", async () => {
        ...
        const res1 = await request(app).post("/api/persons").send(payload1);
        ...
        const res2 = await request(app).post("/api/persons").send(payload2);
        const childId = res2.body.id;

        // Establish relationship PARENT -> CHILD
        const relRes = await request(app).post(`/api/persons/${parentId}/relationships`).send({
        ...
        // Try self-relation
        const selfRelRes = await request(app).post(`/api/persons/${parentId}/relationships`).send({
